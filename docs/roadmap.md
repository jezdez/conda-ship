...
- keep Windows ARM64 as a regular CI canary for `cs` and `cs-template`, and
  publish release assets only after the conda package ecosystem has enough
  stable `win-arm64` coverage for conda-ship runtimes
